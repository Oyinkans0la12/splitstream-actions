...
export async function findMergedPullRequests(
  ...
  if (Number.isNaN(sinceMs)) {
    throw new IngestError(
      `cycle window boundary is not a valid ISO 8601 timestamp: '${sinceIso}'`,
    );
  }
  ...
  for (const { owner, name: repo } of repos) {
    ...
    for (const pr of results) {
      ...
      if (Number.isNaN(mergedAtMs)) {
        warnings.push(
          `PR #${pr.number} in ${owner}/${repo} has an unparseable merged_at '${pr.merged_at}'; skipped`,
        );
        continue;
      }
      if (mergedAtMs < sinceMs) continue; // outside this cycle's window (half-open: the boundary is included)
