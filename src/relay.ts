...
export async function relayCycleRoot(opts: {
  ...
  vault: Contract;
  keypair: Keypair;
  /** The registry's `oracleAccount` — the keypair must resolve to this exact G... key. */
  expectedOracleAccount: string;
  networkPassphrase: string;
  cycleId: number;
  root: Buffer;
  totalAmount: bigint;
  ...
  if (actualOracle !== expectedOracleAccount) {
    throw new RelayError(
      `oracle identity mismatch: ORACLE_SECRET_KEY resolves to ${actualOracle}, ` +
